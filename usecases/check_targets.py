...
class CheckTargetsUseCase:
    ...
    async def check_trigger(self, price: CryptoPrice):
        if ((price.movement_direction.DOWN and price.last_saved > target <= price.target) or
                (price.movement_direction.UP and price.last_saved < target <= price.current)):
